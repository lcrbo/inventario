...
<x-contenttitulo>{{ 'Conversión Activos' }}</x-contenttitulo>

  <x-crudlist>
    <x-slot name="titulo">{{ 'Lista de conversión Activos' }}</x-slot>
    ...
    <x-slot name="tbody">
        ...
        <x-crudtbody>{{ $movimientoingreso->tipodocumento->descripcion }} [{{ $movimientoingreso->numerodocumento }}]</x-crudtbody>
          ...
          <x-crudbotones>
            ...
            <x-boton color="red" url="{{ route('movimientoingresos.edit', $movimientoingreso->id) }}"> Conversión 
