...
<x-slot name="slot">
  <x-contenttitulo>{{ 'Despachos' }}</x-contenttitulo>
<x-crudform>
    <x-slot name="titulo">Modificar Despacho</x-slot>
    <x-slot name="subtitulo">[ {{$despacho->descripcion }} ].</x-slot>
    <x-slot name="action">{{ route('despacho.update', $despacho->id) }}</x-slot> 
