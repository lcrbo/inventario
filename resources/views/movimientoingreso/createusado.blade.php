...
<x-contenttitulo>{{ 'Activos Usados' }}</x-contenttitulo>

<x-crudform>
    <x-slot name="titulo">Incorporar activo Usado</x-slot>
    <x-slot name="subtitulo"></x-slot>
    <x-slot name="action">{{route('movimientoingresos.storeusado')}}</x-slot> 
      ...
      <x-combo nombre="Estado" campo="estado_stock_id" >
        ...
        <x-slot name="condicion">
            @foreach($data['estadostocks'] as $estadostock)
                  @if ($estadostock->descripcion ==  "EN REVISIÓN")
